...
*There is an individual with a well-behaved utility function, and initial wealth $Y$. Let a lottery offer a payoff of $G$ with probability $\pi$ and a payoff of $B$ with probability $1-\pi$.* [@exercises_danthine, p.11]

a. *If the individual already owns this lottery denote the minimum price he would sell it for by $P_s$. Write down the expression $P_s$ has to satisfy.* [@exercises_danthine, p.11]

b. *If he does not own it, write down the expression $P_b$ (the maximum price he would be willing to pay for it) has to satisfy.* [@exercises_danthine, p.11]

c. *Assume now that $\pi=1/$2, $Y=10$, $G=6$, $B=26$, and the utility function is $U(Y)=Y^{1/2}$. Find buying and selling prices. Are they equal? Explain why not. Generally, can they ever be equal?* [@exercises_danthine, p.11]
